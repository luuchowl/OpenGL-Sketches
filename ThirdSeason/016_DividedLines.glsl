...
void main()
{
    ...
    pulse += sin(u_time) * 0.2;
    pulse += cos(u_time *0.4) * 0.15;
    pulse *= 3.0; 
    st.x += (0.5 - st.x) * pow(distance(st.y , 0.5), 2.1 + pulse * 0.15 * (1.-distance(st.y , 0.5)))  * (-27.0 + pulse  * 10.5 * pow((1.-distance(st.y , 0.5)), 5.));
    float n = 50.0;
    float x = floor(st.x * n) / n;
    float t = u_time * 0.2+ sin(u_time* 2.4 + random(x) * 3.141592) * 0.05;

    float linet = fract(t - random(x));
    float offset = fract(1.- st.y - quartic(linet) );
    float light = pow(offset, random(x) * 3.0);



    vec3 color = mix(vec3(0.6, 0.5, 0.8), vec3(.02, 0.9, 0.9), sin(t * 4.0 + random(x * 30.) * 6.70)) *light;
    color += vec3(0.145, 0.03, 0.0);
