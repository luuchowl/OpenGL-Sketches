...
{
  vec3 cubePoint = ((rotateY(0.0)) * vec4(samplePoint, 1.0)).xyz;
   cubePoint = samplePoint;
  float n = noise(vec2(samplePoint.x * 0.1 + sin(u_time) * 1.0, samplePoint.z * 0.1 + cos(u_time) * 3.0 ) * 2.0);

  vec3 p = samplePoint ;
  //p = (rotateY(u_time) * vec4(samplePoint, 1.0)).xyz;
  p = p + n * 1.0;
  p.z = fract(samplePoint.z / 8.0) * 4.7;
  float cube = cubeSDF(abs((vec4(p ,1.0)).xyz) - vec3(4.0, 0.0, 0.0), vec3(0.5, 0.5, 8.0));


  float shape = cube;
  for(int i = 0; i < 6; i++)
  {
    float trail = cubeSDF(abs(p ) - vec3(0.0, 0.0, float(i) * 1.5), vec3(5.5, 0.3, 0.3));
    shape = unionSDF(trail, shape);
    //shape = trail;
  }
  ...
  //  for(int j = -5; j < 5; j++)
  //  {
  //    cubePoint =  samplePoint-vec3(float(i) * 1.5, 0.0, float(j) * 1.5 );
  //    cubePoint = ((rotateY(-u_time * 1.0 +  0.15 * length(vec2(float(i),float(j))))) * vec4(cubePoint, 1.0)).xyz;
...
void main()
{
    ...
    {
        //Didn't hit anything]
        gl_FragColor = vec4(0.0, 0.3, 0.5, 1.0) * viewDir.y * 3.;
    ...
    if(dist>30.0)
    {
      ...
      vec3 K_a = vec3(0.2, 0.55, 0.2);
      vec3 K_d = vec3(0.3, 0.1, 0.3);
      vec3 K_s = vec3(0.0, 11.1, 0.7);
    ...
    vec3 K_a = vec3(0.2, 0.2, 0.5);
    vec3 K_d = vec3(0.5, 0.5, 0.6);
    vec3 K_s = vec3(0.8, 80.6, 1.8);
    float shininess = 10.0;
