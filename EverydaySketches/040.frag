...
void main()
{
  ...
  st = rotate2D(st,PI*0.25 + u_time * 0.1);
  st *= 2.0;
  st -= vec2(1.0);
  ...
  //st.x += (n * 5.0 * fract(u_time * 2.0 / 100.0)) - 0.5;
  st.y += m * 5.9;
  float shape = 0.0;
...
//st = rotate2D(st,PI*0.25 + u_time);
  ...
  for(int i = 0; i < particles; i++ )
  {
      ...
      //st.y += n * 0.0011 * (float(particles) -  lifetime);
      ...
      sdf += lifetime * lifetime * lifetime;
      float circle = smoothstep(sdf, size -0.5, size);
      circle = clamp(circle, 0.0, 1.0);
      //shape = min(circle, shape) * 1.8 ;
      shape += (1.0-circle) * lifetime;
