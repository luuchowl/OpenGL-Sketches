...
void main()
{
  ...
  st *= 1.0;
  //st.y *= 20.0;
  float n = noise(floor(st * 2.0 + st.y) + vec2(sin(u_time * 1.5) * 2.6, 0.0));
  ...
  float shape2 = box(fract(st),vec2(oscilate(0.2, 0.8, 5.0, _st.y * 10.5 ) ), 0.10);
  //vec2 stcircle = st;
  ...
  color.b = max(color.b, oscilate(0.3, 0.7, 3.0, 0.0));
  color.r = min(color.r, 0.9);
  if(color.b < 0.5)
  {
  //  color.rg =vec2( color.r * color.g);
